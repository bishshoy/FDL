...
import numpy as np
import math
from mnist_net import Net, BigNet, EnsembleNet
...
def train_joint_model(args, bignet, device, train_loader, epoch):
    ...
    optimizer1 = optim.SGD(
        bignet.model1.parameters(), lr=1e-2, momentum=0.9, weight_decay=1e-4
    )
    optimizer2 = optim.SGD(
        bignet.model2.parameters(), lr=1e-2, momentum=0.9, weight_decay=1e-4
    )
    accuracy_diff_optimizer = optim.SGD(
        bignet.parameters(), lr=1e-2, momentum=0.9, weight_decay=1e-4
    )
    all_optimizer = optim.SGD(
        bignet.parameters(), lr=1e-2, momentum=0.85, weight_decay=1e-4
    )
    similarity_optimizer = optim.SGD(
        bignet.parameters(), lr=1e-4, momentum=0.85, weight_decay=1e-4
    )
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if epoch % 5 == 0:
            zero_grad_all_optimizers()
            (output1, x1_1, x2_1), (output2, x1_2, x2_2) = bignet(data)
            loss12 = -1 * (similarityLoss(x1_1, x1_2) + similarityLoss(x2_1, x2_2))
            loss12.backward()
            similarity_optimizer.step()
        else:
            loss12 = torch.zeros(1)
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            try:
                x1_1, x1_2 = x1
                x2_1, x2_2 = x2
                loss12 = similarityLoss(x1_1, x1_2) + similarityLoss(x2_1, x2_2)
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        dataset1, batch_size=256, num_workers=4, shuffle=True, pin_memory=True
    )
    test_loader = torch.utils.data.DataLoader(
        dataset2, batch_size=10000, num_workers=4, shuffle=False, pin_memory=True
    )

    # print('Model 1::')
    # model1 = Net().to(device)
    # model1.load_state_dict(torch.load('./saved-models/mnist-net.ckpt'))
    # test(model1, device, test_loader)
    #
    # optimizer1 = optim.Adadelta(model1.parameters(), lr=1.0)
    # scheduler = StepLR(optimizer1, step_size=1, gamma=0.7)
    # for epoch in range(20):
    #     train(args, model1, device, train_loader, optimizer1, epoch)
    #     accuracy = test(model1, device, test_loader)
    #     scheduler.step()
    ...
    # scheduler1 = StepLR(optimizer1, step_size=1, gamma=0.7)
    ...
    for epoch in range(1, 100000):
        ...
        accuracy = test(ensemblenet, device, test_loader)
